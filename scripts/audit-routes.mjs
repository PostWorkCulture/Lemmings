import {solvePuzzle} from '../tests/puzzle-solutions.js';
...
for(const index of Array.from({length:15},(_,i)=>i+5)){
 const g=new Game(index);if(g.level.puzzleId){const solved=solvePuzzle(index).g;console.log(index+1,solved.result,solved.saved,solved.lost);continue;}const remove=g.remove.bind(g);g.remove=(u,saved)=>{if(!saved)console.log("PIONEER LOST",index+1,g.tick,JSON.stringify(u));remove(u,saved);};let job=0,held=false,released=false,lastAction=0;
