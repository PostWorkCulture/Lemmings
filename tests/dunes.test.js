...
test('a steep dune sends walkers and climbers sliding back down',()=>{
 ...
 const u={id:0,x:428,y:295,state:'walk',dir:1,abilities:{climb}};g.units=[u];g.step();assert.equal(u.state,'slide');
 for(let n=0;n<15;n++)g.step();assert.ok(u.x<411);assert.ok(u.y>325);assert.equal(u.dir,-1);
 }
});
