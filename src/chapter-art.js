...
export function chapterBackdrop(c,key,height,level=null){
 ...
 if(key==='circus'){
  ...
  tent(tents,125,ground,230,Math.min(220,height-120));tent(tents,875,ground,230,Math.min(220,height-120));tent(tents,500,ground,420,Math.min(360,height-90));
  c.save();c.globalAlpha=.12;c.drawImage(layer,0,0);c.restore();
 }else{
...
export function mountainWeather(c,t,height){
 ...
 for(let i=0;i<1;i++){const p=((t*.0015+i*.33)%1),peak=125+i*330,top=80+((1+i*2)%3)*40,x=peak+p*180,y=top+p*(430-top);
  c.save();c.translate(x,y);c.scale(.7,.7);c.rotate(Math.atan2(430-top,180));c.strokeStyle='#345c73';c.lineWidth=2.5;c.beginPath();c.moveTo(-7,5);c.quadraticCurveTo(0,8,8,5);c.stroke();c.strokeStyle=['#dc5378','#386fe2','#e59339'][i];c.lineWidth=4;c.beginPath();c.moveTo(0,2);c.lineTo(-2,-7);c.stroke();oval(c,-2,-11,3,3,'#e5eef0');c.restore();
  for(let j=0;j<3;j++)oval(c,x-7-j*3,y+3-j,1.3,1,'#e9faffaa');
 }
 for(let layer=0;layer<1;layer++)for(let i=0;i<12;i++){
