...
export class Game {
  ...
  at(x,y) { x=Math.floor(x);y=Math.floor(y);if(x<0||x>=WIDTH||y<0||y>=this.height)return 0;for(const o of this.level.objects||[])if(o.type==='lift'){const p=objectPosition(o,this.tick);if(x>=p.x&&x<=p.x+o.w&&y>=Math.floor(p.y)&&y<Math.floor(p.y)+5)return 2;}return this.terrain[y*WIDTH+x]; }
  spawn() { this.lastSpawnTick=this.tick;const u={id:this.spawned++,x:this.level.spawnX,y:this.level.spawnY,dir:this.level.dir,state:'fall',vy:0,fallStart:this.level.spawnY,jobTick:0,steps:0};this.units.push(u);return u; }
  canAssign(u,skill) {
    ...
    for(const u of this.units) {
      ...
      if(u.state==='bash'||u.state==='mine'){
        if(++u.jobTick%8===0){const mining=u.state==='mine',nx=u.x+u.dir*3,ny=u.y+(mining?2:0);let steel=false,earth=false;
          for(let y=ny-22;y<ny+(mining?3:0);y++)for(let x=nx-7;x<nx+8;x++){const t=this.at(x,y);if(t===2)steel=true;if(t===1)earth=true;}
